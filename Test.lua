...
-- Returns List Of Actual Unit Objects
function getGroupLeadsFromMission(sideName,mission)
    local unitKeyValue = {}
    local returnUnits = {}
    if mission and mission.unitlist then
        for k,v in pairs(mission.unitlist) do
            local unit = ScenEdit_GetUnit({side=sideName, guid=v})
            if unit then
                if unit.group then
                    if (unit.guid == unit.group.lead) then
                        returnUnits[#returnUnits + 1] = unit
                    end
                else
                    returnUnits[#returnUnits + 1] = unit
                end
...
--------------------------------------------------------------------------------------------------------------------------------
function determineEvasionInReconMissions(sideShortKey,sideAttributes,side,missionNames,retreatRange)
    local currentTime = ScenEdit_CurrentTime()
    local nextMissionTime = getTimeStampForKey(sideShortKey.."_nxt_rec_eva_mission_ts")
    local resetMissionTime = false
    for k,v in pairs(missionNames) do
        local mission = ScenEdit_GetMission(side.name,v)
        local missionUnits = getGroupLeadsFromMission(side.name,mission)
        for k1,missionUnit in pairs(missionUnits) do
            if missionUnit and missionUnit.speed > 0 and (not determineUnitRTB(side.name,missionUnit.guid)) then
                local unitRetreatPoint = getEmergencyMissileNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,missionUnit)
                if unitRetreatPoint then
                    determineEvasionRoute(side,missionUnit,unitRetreatPoint,false)
                else
                    if (nextMissionTime - currentTime) <= 0 then
                        resetMissionTime = true
                        unitRetreatPoint = getAirNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,missionUnit,retreatRange)
                        if unitRetreatPoint then
                            determineEvasionRoute(side,missionUnit,unitRetreatPoint,true)
                        else
                            unitRetreatPoint = getSAMNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,missionUnit)
                            if unitRetreatPoint then
                                determineEvasionRoute(side,missionUnit,unitRetreatPoint,true)
                            else
                                unitRetreatPoint = getShipNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,missionUnit)
                                if unitRetreatPoint then
                                    determineEvasionRoute(side,missionUnit,unitRetreatPoint,true)
                                end
                            end
    ...
    if resetMissionTime then
        setTimeStampForKey(sideShortKey.."_nxt_rec_eva_mission_ts",tostring(currentTime + 30))
    end
end

function determineEvasionInFighterMissions(sideShortKey,sideAttributes,side,missionNames,retreatRange)
local currentTime = ScenEdit_CurrentTime()
    local nextMissionTime = getTimeStampForKey(sideShortKey.."_nxt_fig_eva_mission_ts")
    local resetMissionTime = false
    for k,v in pairs(missionNames) do
        local mission = ScenEdit_GetMission(side.name,v)
        local missionUnits = getGroupLeadsFromMission(side.name,mission)
    ...
    if resetMissionTime then
        setTimeStampForKey(sideShortKey.."_nxt_fig_eva_mission_ts",tostring(currentTime + 30))
    end
end

function determineEvasionInStealthFighterMissions(sideShortKey,sideAttributes,side,missionNames,retreatRange)
    local currentTime = ScenEdit_CurrentTime()
    local nextMissionTime = getTimeStampForKey(sideShortKey.."_nxt_sfig_eva_mission_ts")
    local resetMissionTime = false
    for k,v in pairs(missionNames) do
        local mission = ScenEdit_GetMission(side.name,v)
        local missionUnits = getGroupLeadsFromMission(side.name,mission)
    ...
    if resetMissionTime then
        setTimeStampForKey(sideShortKey.."_nxt_sfig_eva_mission_ts",tostring(currentTime + 30))
    end
end

function determineEvasionInSupportMissions(sideShortKey,sideAttributes,side,missionNames,retreatRange)
    local currentTime = ScenEdit_CurrentTime()
    local nextMissionTime = getTimeStampForKey(sideShortKey.."_nxt_sup_eva_mission_ts")
    local resetMissionTime = false
    for k,v in pairs(missionNames) do
        local mission = ScenEdit_GetMission(side.name,v)
        local missionUnits = getGroupLeadsFromMission(side.name,mission)
    ...
    if resetMissionTime then
        setTimeStampForKey(sideShortKey.."_nxt_sup_eva_mission_ts",tostring(currentTime + 30))
    end
end

function determineEvasionRoute(side,missionUnit,unitRetreatPoint,allowDisable)
    if allowDisable and not unitRetreatPoint then
        ...
        if unitRetreatPoint then
            if missionUnit.group and missionUnit.group.unitlist then
                for k,v in pairs(missionUnit.group.unitlist) do
                    local subUnit = ScenEdit_GetUnit({side=side.name,guid=v})
...
function getAirNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,unit,desiredRange)
    local hostileAirContacts = getHostileAirContacts(sideShortKey)
    local unknownAirContacts = getUnknownAirContacts(sideShortKey)
    if not unit then
        return nil
    end
    for k,v in pairs(hostileAirContacts) do
        ...
        if contact then
            local currentRange = Tool_Range(contact.guid,unit.guid)
...
function getSAMNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,unit)
    local hostileSAMContacts = getHostileSAMContacts(sideShortKey)
    if not unit then
        return nil
    end
...
function getShipNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,unit)
    local getHostileSurfaceShipContacts = getHostileSurfaceShipContacts(sideShortKey)
    if not unit then
        return nil
    end
...
function getEmergencyMissileNoNavZoneThatContainsUnit(sideShortKey,sideAttributes,side,unit)
    local hostileMissilesContacts = getHostileWeaponContacts(sideShortKey)
    if not unit then
        return nil
    end
    for k,v in pairs(hostileMissilesContacts) do
        local currentRange = Tool_Range(v,unit.guid)
        local contact = ScenEdit_GetContact({side=side.name, guid=v})
        if contact then
            ...
            if currentRange > minDesiredRange and currentRange < maxDesiredRange then
                ...
                local bearing = Tool_Bearing({latitude=contactPoint.latitude,longitude=contactPoint.longitude},unit.guid)
...
function actorUpdateUnitsInReconMission(args)
    ...
    -- Determine Evasion
    determineEvasionInReconMissions(args.shortKey,args.options,side,missions,80)
...
function actorUpdateUnitsInOffensiveAirMission(args)
    ...
    -- Determine Evasion
    determineEvasionInFighterMissions(args.shortKey,args.options,side,missions,100)
...
function actorUpdateUnitsInOffensiveStealthAirMission(args)
    ...
    -- Determine Evasion
    determineEvasionInStealthFighterMissions(args.shortKey,args.options,side,missions,70)
...
function actorUpdateUnitsInOffensiveAEWMission(args)
    ...
    -- Determine Evasion
    determineEvasionInSupportMissions(args.shortKey,args.options,side,missions,200)
end
